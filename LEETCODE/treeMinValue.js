...
const treeMinValue = (root) => {
    return treeMinValueDFS(root);
  };
  const treeMinValueDFS = (root, min = Infinity)=>{
    if(root == null)return min;
    let current = root.val;
    if(current < min){
      min = current;
    }
    min = treeMinValueDFS(root.left,min);
    min = treeMinValueDFS(root.right,min);
